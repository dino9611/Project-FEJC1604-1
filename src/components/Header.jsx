    ...
    return (
      <div className="header-bg">
        ...
        <div className="header-menu" >
          <Link to="/collection" className="normal-link-header" style={{ marginRight: '30px', letterSpacing: '1px', fontSize: '20px' }}>
            <a>Collection</a>
          </Link>
          {this.props.dataUser.islogin && this.props.dataUser.role === 1 ? (
            <Fragment style={{ marginRight: '15px' }}>
              ...
              {this.props.dataUser.cart.length ?
                <Badge pill style={{ background: '#89ADC3', position: 'relative', bottom: 7, right: 7 }} >
                  {this.props.dataUser.cart.length}
                </Badge>
                :
                null
          ...
          {this.props.dataUser.islogin && this.props.dataUser.role === 1 ?
            <Dropdown isOpen={this.state.isOpen} toggle={this.toggle} style={{ outline: 'none' }}>
              ...
              <DropdownMenu>
                <DropdownItem><Link to='/userprofile' style={{ textDecoration: 'none', color: '#052c43' }}>My Profile</Link></DropdownItem>
                <DropdownItem ><Link to='/history' style={{ textDecoration: 'none', color: '#052c43' }}>History</Link></DropdownItem>
                <DropdownItem><Link to='/payment' style={{ textDecoration: 'none', color: '#052c43' }}>Payment</Link> </DropdownItem>
                <DropdownItem divider />
                <DropdownItem>
                  <Link to="/" className="normal-link-header" onClick={this.onLogoutClick} style={{ color: '#052c43' }}>
                    Log Out
                  </Link>
                </DropdownItem>
              </DropdownMenu>
