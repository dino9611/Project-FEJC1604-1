...
class AddressList extends Component {
    ...
    renderAddress = () => {
        return this.state.addresses.map((val, index) => {
            return (
                <tr key={index} style={{ backgroundColor: val.is_default == 1 ? '#D4EAF5' : 'white' }}>
                    <td>{index + 1}</td>
                    <td>{val.address}</td>
                    <td>{val.city}</td>
                    <td>{val.zip}</td>
    ...
    onAddClick = () => {
        ...
        if (city && address && zip && description) {
            axios
                .get(`http://api.positionstack.com/v1/forward`, { params: { access_key: '82fd700377254a8002ecb47ba7b9d5c7', query: this.state.addAddress.city } })
                .then((res) => {
                    console.log(res.data);
                    if (res.data.data.length) {
                        ...
                        axios.post(`${API_URL}/auth/addAddress/${this.props.Auth.id}`, data)
                            .then((res2) => {
                                this.setState({
                                    addresses: res2.data,
                                    modalVisible: false,
    ...
    render() {
        return (
            <div>
                <Modal isOpen={this.state.modalVisible} toggle={this.toggle} centered size='xl'>
                    <ModalHeader toggle={this.toggle} >Add Address</ModalHeader>
                    <ModalBody>
                        <input placeholder="City" className="form-control my-3" name="city" value={this.state.addAddress.city} onChange={this.onInputChange} />
                        <input placeholder="ZIP" className="form-control" name="zip" value={this.state.addAddress.zip} onChange={this.onInputChange} />
                        <textarea placeholder="Address" className="form-control my-3" rows='5' name="address" value={this.state.addAddress.address} onChange={this.onInputChange}></textarea>
